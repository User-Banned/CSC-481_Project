...
# matching the preferred primary storage size, and with the desired RAM size
def recommend_laptops_with_model(max_price_limit, min_rating, preferred_storage_size, desired_ram_size, cluster_indices, num_results=5):
    recommended_laptops = []
    num_found = 0
    ...
    for idx in cluster_indices:
        ...
        if (min_rating is None or rating[idx] >= min_rating) and \
           ...
           (desired_ram_size is None or ramSize[idx] == desired_ram_size):
...
# Prompt the user for their price limit, preferred minimum rating, their preferred RAM size, preferred primary storage size
...
# Use k-means to find the cluster for the given price, rating, and preferred storage size
user_features = [price_limit, min_rating, 0, 0, preferred_ram_size, preferred_storage_size, 0, 0, 0, 0]  # Placeholder value for missing features
if min_rating is None:
    user_features[1] = 0  # Placeholder value for missing minimum rating
if preferred_ram_size is None:
    user_features[4] = 0  # Placeholder value for missing preferred storage size
...
recommended_laptops = recommend_laptops_with_model(price_limit, min_rating, preferred_storage_size, preferred_ram_size, cluster_indices, 5)

if recommended_laptops:
    print("Recommended laptops within your price limit, with a rating over", min_rating, ", matching your preferred primary storage size, and with a RAM size of", preferred_ram_size, "GB:")
